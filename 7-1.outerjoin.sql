...
-- left join을 하고싶으면 매칭되는 오른쪽 칼럼에 (+) 더해주기
...
-- # right outer join
-- 조인되는 오른쪽 테이블은 모두조회하고, 왼쪽 테이블은 조건에 매칭된 것만 조회

-- **모든 부서정보 조회** + 혹시 부서코드가 일치하는 사원(소속된 사원)이 있으면 같이 조회
SELECT 
    A.emp_no, A.emp_nm, A.dept_cd, b.dept_nm
FROM tb_emp A
...
-- #full Outer join
-- left+right
-- 왼쪽 조인테이블, 오른쪽 조인 테이블을 모두 보여주고 매칭되면 매칭 데이터까지 합쳐서 보여줌
-- 41 + 5(부서없는사원X) + 2(사원 없는 부서) 합쳐져서 나옴
