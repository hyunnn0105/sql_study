...
-- 들어있는 데이터가 동일(글자가 똑같으면)하면 글자의 공백(길이)과 상관없이 true가 나온다
SELECT 
    REPLACE(char_4, ' ', '+') AS char_4,
    REPLACE(char_6, ' ', '+') AS char_6
FROM char_compare
...
SELECT 
    ...
    SUBSTR('SQL Developer', 1, 3), -- 문자열 자름 1번부터 3개자름 (첫글자가 1번) : SQL
    LENGTH('HELLO WORLD'),         -- 문자열의 길이(공백이 가운데면 글자수에 포함함)
    TRIM('    HI   ')              -- 좌우 공백 제거
FROM dual;

SELECT 
    RPAD('Steve', 10, '-'), -- R: 오른쪽에 주어진 문자를 채움
    LPAD('Steve', 10, '*'),  -- L: 왼쪽에 주어진 문자를 채움
...
-- 날짜 - 날짜 = 일수  => 어떤 날짜에서 다른 날짜를 뺀 일수 (사이간격)
